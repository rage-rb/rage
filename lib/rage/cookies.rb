...
class Rage::Cookies
  ...
  def delete(key, path: "/", domain: nil)
    ...
    @headers["Set-Cookie"] = Rack::Utils.add_cookie_to_header(nil, key, {
      value: "".freeze, expires: Time.at(0), path: path, domain: domain
    })
  end
  ...
  def []=(key, value)
    ...
    unless value.is_a?(Hash)
      ...
      @headers["Set-Cookie"] = Rack::Utils.add_cookie_to_header(nil, key, { value: serialized_value, expires: @expires })
      return
    end
  ...
  def request_cookies
    ...
    @parsed = true
    if cookie_header = @env["HTTP_COOKIE".freeze]
      cookie_header.split(/; */n).each do |cookie|
        next if cookie.empty?
        key, value = cookie.split("=".freeze, 2).yield_self { |k, _| [k.to_sym, _] }
  ...
  class EncryptedJar
    ...
    class << self
      ...
      def dump(value)
        ...
        Base64.urlsafe_encode64("00".freeze + primary_box.encrypt(value.to_s))
      end
