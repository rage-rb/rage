...
# • _config.logger_
#
# > The logger used by `Rage.logger`. Accepts a logger object that implements the `#debug`, `#info`, `#warn`, `#error`, `#fatal`, and `#unknown` methods, or `nil`. If set to `nil`, logging will be disabled.
...
# > ### External Logger
#
# > You can set an external logger by passing an object that responds to `#call`. This allows you to send Rage's raw structured logging data directly to external observability platforms without serializing it to text first.
#
...
#
# > The `#call` method receives the following keyword arguments:
#
# > - `severity` - the log severity as a symbol (one of `:debug`, `:info`, `:warn`, `:error`, `:fatal`, or `:unknown`).
# > - `tags` - an array of log tags submitted via {Rage::Logger#tagged Rage.logger.tagged}. The first tag in the list is always the request ID.
# > - `context` - a hash containing the log context submitted via {Rage::Logger#with_context Rage.logger.with_context}.
# > - `message` - the log message. For request logs generated by Rage, the message is always `nil`.
# > - `request_info` - a hash containing request-specific information. The value is `nil` for non-request logs and contains the following keys for request logs:
