...
RSpec.describe Rage::Deferred::Queue do
  ...
  before do
    ...
    allow(Rage::Deferred::Context).to receive(:get_task).with(task_context).and_return(task_class)
  ...
  describe "#enqueue" do
    ...
    it "schedules the task for execution" do
      expect(subject).to receive(:schedule).with("task-id", task_context, publish_in: nil)
      subject.enqueue(task_context)
    end
    ...
    context "with backpressure enabled" do
      ...
      it "raises a timeout error if backpressure timeout is reached" do
        ...
        expect { subject.enqueue(task_context) }.to raise_error(Rage::Deferred::PushTimeout, "could not enqueue deferred task within 0.02 seconds")
      end
    end
  end
  ...
  describe "#schedule" do
    ...
    it "performs the task" do
      expect(task_instance).to receive(:__perform).with(task_context).and_return(true)
      subject.schedule("task-id", task_context)
    end
    ...
    context "with a delay" do
      it "schedules with a delay in milliseconds" do
        ...
        subject.schedule("task-id", task_context, publish_in: 5)
      end

      it "does not affect backlog size" do
        expect { subject.schedule("task-id", task_context, publish_in: 5) }.not_to change { subject.backlog_size }
      end
    end
    ...
    context "without a delay" do
      ...
      it "increments and decrements backlog size" do
        expect { subject.schedule("task-id", task_context) }.to change { subject.backlog_size }.by(0)
